...
def get_commit_tree(db: DataBase, commit_sha: Sha) -> tuple[Sha, Tree]:
...
def checkout_use_case(branch_name: str, repository_root_dir: Path = Path(".")) -> None:
    ...
    current_tree_sha, current_tree = get_commit_tree(db, current_commit_ref.sha)
    target_tree_sha, target_tree = get_commit_tree(db, target_commit_ref.sha)

    tree_diff_service = TreeDiffService(current_tree_sha, target_tree_sha)
